...
| 9 | 2026-09-14 · l'allarme sui SOLDI del Guardiano aveva UN canale (email, solo sull'anomalia) e col provider spento moriva in un log senza nemmeno contare; il battito diceva «vivo», non «cosa ho trovato» | la domanda del fondatore «chi li ripara?» rovesciata in «chi li RICEVE?», e un censimento che per ogni `logger.critical` ha cercato l'handler che lo inoltra (nessuno: solo stdout e file) | il tick lascia l'ESITO su disco e il watchdog (Telegram, ogni 10 min) lo grida finché resta anomalo; il rapporto parte ogni giorno; provider spento = WARNING + `email_ko` (esistono: `TestLEsitoDelGuardianoArrivaAUnaPersona`, `TestIlTickLasciaLEsitoEIlRapportoArrivaSEMPRE` col tick vero, viste rosse prima) | sì per il Guardiano; la famiglia «CRITICAL invisibile a `_guasti_isolati`» chiusa la sera stessa (riga 11) |
...
| 11 | 2026-09-14 · `fase186._guasti_isolati` filtrava solo ` ERROR `: le righe ` CRITICAL ` (Bunker violato, kill-switch, cancellazione forzata) — il livello più grave — erano le uniche che nessun lettore automatico vedeva | un censimento che per ogni `logger.critical` del prodotto vivo ha cercato CHI lo legge, e ha confrontato la parola scritta da `main_casavip` (`%(levelname)s`) con la parola filtrata | una guardia che scrive una riga CRITICAL fresca nel registro e pretende l'allarme (esiste: `test_il_livello_CRITICO_conta_come_un_guasto_e_non_di_meno`, vista rossa prima) | sì |
| 12 | 2026-09-14 · una controversia aperta dall'ospite non avvisava nessuno: né ERROR né email, e il Guardiano cerca le garanzie ferme in `in_garanzia`, non in `contestato` | la domanda «chi se ne accorge?» applicata rotta per rotta al fronte notifiche | una guardia sul cablaggio: la rotta deve produrre un ERROR e un'email a chi decide, provider spento → `email_ko` (esiste: `TestUnaControversiaApertaAvvisaQualcuno`, vista rossa prima) | per l'esemplare sì; la famiglia «stato di denaro fermo che il Guardiano non censisce» (`contestato`) resta da chiudere in `fase186` |
...
| 17 | 2026-09-15 · le tre caselle prezzo del pannello host partivano GIÀ SCRITTE (`p_prezzo` e `d_prezzo` con un numero, `r_prezzo` con un altro) dal commit `ae36668` del 13/7, mai tolte: chi non le cambia vende alla cifra di qualcun altro. Il fondatore ha caricato un annuncio a 1 € e il periodo si è aperto alla cifra della casella | il FONDATORE che usa il pannello come un host vero (30 minuti), non un test: nessuno dei ~6800 percorreva il pannello nell'ordine di una persona | una guardia che LEGGE dal pannello quali caselle mandano `prezzo_notte_cents`/`prezzo_netto_cents` e pretende che partano vuote — e che ognuna rifiuti il vuoto, perché `BV.toCents('')` vale zero (esiste: `TestNessunaCasellaDiPrezzoPARTEGIASCRITTA`, vista ROSSA prima: `['d_prezzo=95', 'p_prezzo=95', 'r_prezzo=90'] != []`) | sì: gli id non sono scritti a mano, una casella di prezzo nuova entra da sola nel perimetro. ⚠️ resta fuori la posizione del rifiuto (prima o dopo la chiamata): la vede il giro col browser |

| 18 | 2026-09-15 · le scritture dell'host riuscivano su un annuncio CHE NON ESISTE: `_verifica_proprieta` risponde `owner is None or owner == hid`, e `owner is None` è anche il nome mai creato. Misurato dalla guardia: `/api/host/disponibilita` → `200 {'stato': 'ok'}`, `/api/host/disponibilita_range` → `200 {'giorni_impostati': 4}`, `/api/host/ical` → `200 {'eventi': 1, 'giorni_bloccati': 2}` | il fondatore che carica il periodo PRIMA di salvare l'annuncio (nginx: 18:41:48 e 18:42:03) e legge «✅» mentre i giorni vanno altrove; la rotta iCal non l'aveva mai guardata nessuno, ed è uscita insieme alle altre perché la guardia interroga la FAMIGLIA | una guardia che chiama OGNI scrittura host col nome nel corpo usando un nome inesistente e pretende una risposta non-2xx, più il conto dei chiamanti di `_verifica_proprieta` che grida se ne nasce uno nuovo (esiste: `TestUnaScritturaSuUnAnnuncioCHENONESISTE`, vista ROSSA prima con le tre righe qui accanto) | sì per le scritture col nome nel corpo: `_alloggio_in_catalogo` le ferma tutte e tre con 404 e il conto del perimetro impedisce che una rotta nuova resti fuori in silenzio. ⚠️ restano fuori le LETTURE che passano dallo stesso controllo (prezzi, link iCal) |

| 19 | 2026-09-15 · eliminare un annuncio lasciava in archivio il suo CALENDARIO (`fase57_vetrina.elimina_alloggio` toglie solo annuncio e foto): un annuncio ricreato con lo STESSO nome ereditava i giorni del morto, e `rispecchia_prezzo` metteva in vetrina il prezzo del morto. In produzione due calendari orfani, 60 e 59 giorni | il fondatore che elimina e ricrea con lo stesso nome — il gesto normale di chi sbaglia un annuncio, che nessun collaudo percorreva | una guardia che apre un periodo dalla ROTTA vera, elimina e pretende il calendario vuoto, più una che ricrea con lo stesso nome e pretende il prezzo appena scritto (esistono in `test_elimina_annuncio`, viste ROSSE prima: `10 != 0` e `9000 != 100`) | sì, e la famiglia era già nominata: `svuota_calendario` (fase58) toglie giorni, blocchi esterni e stato messo da parte, e **lascia `movimenti`**, cioè lo storico delle prenotazioni che l'host legge nel pannello. ⚠️ resta aperta la variante «nome riusato»: lo storico e le garanzie restano legati al NOME, quindi un altro host che ricrea quel nome se li troverebbe accanto — da chiudere |

| 20 | 2026-09-15 · il pannello host chiamava «Trattenuto (cancellata)» — in tutte e 8 le lingue — uno stato che scrivono TRE cause diverse: prenotazione cancellata, controversia aperta, subentro. Su un rimborso pieno il fondatore leggeva «trattenuto» di soldi che nessuno stava tenendo, e la legenda degli incassi spiegava gli altri tre stati e saltava proprio quello | il fondatore che legge il SUO pannello e chiede «perché mi trattenete questi soldi?»: la cifra era giusta (la sua quota), la parola no | una guardia che lega il TESTO al CODICE — conta i punti che scrivono lo stato e, se sono più d'uno, pretende che l'etichetta non nomini una causa sola, in ogni lingua; più una che pretende che la legenda spieghi lo stato con la sua etichetta (esistono in `test_host_calendario_ui`, viste ROSSE prima: `8 != 0` su otto lingue e `it/en` senza legenda) | sì per la famiglia «un'etichetta nomina una causa che non è l'unica»: il conto dei chiamanti è nel codice, quindi se domani una causa sparisce o ne nasce un'altra la guardia lo dice. ⚠️ resta fuori il MOTIVO per singola riga: il payout non lo conserva, e mostrarlo vorrebbe dire scriverlo dove oggi non c'è |

| 21 | 2026-09-15 · la pagina pubblica mostrava «11144.3 km dal centro» su un annuncio con città «test»: il numero è calcolato bene (haversine fra due punti veri) e la frase è falsa — nessun alloggio sta a undicimila chilometri dal centro della SUA città. Modo di rompersi n. 10: formato giusto, numero senza senso | il fondatore che GUARDA la pagina, non un test: i test controllavano che la distanza fosse calcolata bene, e lo era | il tetto di plausibilità dentro `_distanza_centro` — un punto solo per la scheda e per la lista — con lo STESSO confine che il prodotto usa già per scartare un pin assurdo, e una guardia che pubblica un annuncio lontanissimo e pretende che la cifra non esca (esiste in `test_geocoder_mappa`, vista ROSSA prima: `4690085 metri`) | sì per ciò che si MOSTRA. ⚠️ il confine ora è scritto in due punti del file (qui e nella guardia del pin): da unificare in una costante, altrimenti è «valori scritti a mano in più posti» (PARTE 10, direzione 7) |

| 22 | 2026-09-15 · falso rosso in CI su `test_pagina_voucher`: `AssertionError: '1967' unexpectedly found in '<!DOCTYPE html>…`. Il PIN casuale di quel giro, `1967`, coincideva con l'ultimo gruppo del codice prenotazione `BVIP-F279-1967`: il sito non esponeva niente. Il test cercava QUATTRO CIFRE NUDE in tutta la pagina, mentre il prodotto aveva già smesso di farlo — la sua rete difensiva cerca `riga_pin_voucher`, che esiste apposta e lo dichiara nella docstring («~1 su 1500», misurato il 2026-08-15) | la CI stessa, su una PR di soli documenti: il rosso ha fermato un lavoro sano e ha richiesto un secondo giro | il test cerca ora la RIGA del PIN, non le cifre; più due asserzioni che tolgono il verde-per-il-motivo-sbagliato: la forma cercata deve contenere il PIN, e al suo posto nella pagina dev'esserci il lucchetto | per l'esemplare sì. ⚠️ la famiglia «una guardia cerca un valore corto dentro tutta una pagina» resta da chiudere con un controllo che le enumeri: qui c'era già la funzione giusta e nessuno obbligava a usarla |
...
| Dati che sopravvivono al loro padrone (il calendario resta, l'annuncio no) | orphaned data | Alla cancellazione dell'annuncio il calendario si svuota (`fase58.svuota_calendario`), e i movimenti no per scelta dichiarata; guardia che apre, elimina e RILEGGE dalla rotta vera | test | 2026-09-15 |

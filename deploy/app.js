...
(function(){
  ...
  BV.fraseErrore = function(cod, lang){
    ...
    /* ⛔ ULTIMA SPIAGGIA — un codice sconosciuto NON esce MAI in faccia a chi paga.
       Difetto vivo del 2026-08-18, trovato dal percorso col browser: col gateway muto
       l'ospite leggeva `pagamento_non_disponibile` -- il nostro codice interno -- proprio
       mentre stava pagando. Tradurre quel codice e basta avrebbe curato UN caso e lasciato
       viva la malattia: il percorso di prenotazione ne produce ~24 e ognuno nuovo sarebbe
       nato capace di uscire in chiaro. Qui si chiude la CLASSE: chi legge riceve una frase
       vera, e il codice resta nel registro del browser per chi ripara. Guardia:
       test_app_js.TestNessunCodiceInternoInFacciaAllOspite (vista rossa su questa riga). */
    /* ⛔ CODICE ASSENTE -> STRINGA VUOTA, e non e' un dettaglio: chi chiama incatena i
       tentativi (`fraseErrore(r.motivo)||fraseErrore(r.errore)||...` in index.html). Se
       l'assenza rispondesse con la frase generica, la catena si fermerebbe al PRIMO anello
       e la traduzione buona non verrebbe mai raggiunta. Successo davvero il 2026-08-18,
       dieci minuti dopo aver scritto questa funzione: l'ospite leggeva la frase generica al
       posto di quella del pagamento. L'ha visto il percorso col browser, non la lettura. */
    if(cod==null || cod==='') return '';
    try{ console.warn('bookinvip: codice errore non tradotto', cod); }catch(e){}
    return (A && A.generico) || (BV.ERR_AUTH.en && BV.ERR_AUTH.en.generico) || D.server;
  };

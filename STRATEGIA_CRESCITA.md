...
- Piattaforma globale (mix) → prudente **~3,0% + €0,30 a prenotazione**.
...
- Unici costi VERI: **fissi ~€5.400/anno** (INPS del titolare + commercialista; **niente INAIL/costi
  dipendenti** perché non abbiamo dipendenti) e **Stripe** (che dopo il lancio paga l'host). Tutto il
  resto della commissione (8–10%) è **margine**.
- **Pareggio bassissimo**: al 10% basta **~€54k di GMV/anno** per coprire TUTTI i fissi; oltre, ogni euro
  rende **~6% netto puro**. Non c'è stipendio da pagare: **il guadagno del fondatore È l'utile**.
- **Perché i colossi guadagnano pur con costi enormi** (decine di migliaia di dipendenti, ~$17,8 mld/anno
  di marketing): per **SCALA** (miliardi di GMV) + ricavi accessori. **Noi facciamo l'opposto**:
  profittevoli **da subito** con costi quasi nulli → li battiamo su take più basso + fiducia + AI-native.
- ⚠️ **Prossima fase (da controllare)**: classificazione impresa vs professionale col commercialista →
  decide se INPS è minimale fisso (~€2.800–4.300) o a percentuale senza minimo (Gestione Separata, meglio a inizio).

## 6-bis. «Ma loro guadagnano 10× e hanno dipendenti — perché noi meno?»
**Profitto = Volume × Margine.** Loro guadagnano 10× (anzi molto di più) per il **VOLUME**, non per
efficienza: Airbnb ~**$80 mld/anno** di GMV, Booking ~$160 mld → 15% di miliardi = miliardi, pur con
migliaia di dipendenti. **Noi guadagniamo meno SOLO perché il volume è piccolo (siamo all'inizio)**,
non perché il modello è peggiore.
- Il **nostro margine per prenotazione è più ALTO** del loro (zero dipendenti) → **a parità di volume
  guadagneremmo come loro o di più**.
- Unica leva: **far crescere il VOLUME** = il land-grab. I loro dipendenti comprano *scala* (ogni Paese,
  fiducia, offerta); noi la sostituiamo con **automazione + AI** → scaliamo con pochissime persone.
- **Onesto**: non superiamo il loro utile assoluto domani (decennio di vantaggio), ma siamo
...
Fino ad allora: forfettario + questi conti. **Confermare ogni cifra col commercialista prima del go-live.**
